...
利用者から寄せられた要望一覧を、本移行計画の区分（A/A'/B）・実装状況と突き合わせて整理する。
要望一覧内の並び順は要望の粒度がバラバラ（機能単体の要望と「〜系」の見出しが混在）なため
優先度の根拠にはせず、**実装コスト（すでに動くものほど低コスト）と要望の温度感**の両方を
考慮してフェーズ分けする。
...
| 要望 | 対応する移行計画項目 | 現状 |
| --- | --- | --- |
| 要望一覧・開発用環境作成 | （コード実装ではないプロジェクト管理タスク） | 本ドキュメント自体が該当 |
| キーボードヘルプを表示 | 3-2 `help` | 未移植（既存 `keyboard-help` との統合要確認） |
| メニューウィンドウの縮小 | 実装済み `mini-menu`（メニュー最小化） | **対応不要** |
| 回転機能の個別オンオフスイッチ | 実装済み `object-rotate-off-individually` | **対応不要** |
| 保存/コピー時に回転オフが引き継がれない | 計画外（上記機能のバグ） | 要調査・原因切り分けが先 |
| 右クリックでブランクカード作成（+テキスト入力） | 機能A（拡張ブランクカード）は実装済み／テキスト入力は3-1 `add-card-text-writable` | ブランクカード生成自体は対応不要、文字入力のみ未移植 |
| カードに文字入力 | 3-1 `add-card-text-writable` | 未移植（通常カード全般が対象、0-3節参照） |
| 共有メモの直立と並行の切り替え | 実装済み `text-note-upright-flat` | **対応不要** |
| 重ねカード移動機能追加 | 3-1 `move-stacked-card` | 未移植（計画上も低優先度） |
| 手札置き場追加 | 実装済み `use-hand-storage` | **対応不要** |
| 疑似ついたて機能 | 区分A `virtual-screen` ほか | 配線済み（チェックリスト✓）、実機動作確認が未実施 |
| 手札置き場をボードとしてZIP保存 | 計画外・新規要件 | 個人ボードの永続化設計が必要（未着手） |
| コマンド操作で名前表示＆ついたて有効化 | 計画外・新規要件 | 右クリックメニュー仕様の新規設計が必要 |
| 個人ボードの所有権放棄コマンド | 計画外・新規要件 | 上記とセットで設計 |
| 個人ボードをデフォルトで回転可能に | 計画外・小規模 | 初期値変更のみで対応できる可能性 |
| 「手札置き場」→「ボード」に名称変更 | 計画外・表記統一 | ラベル文言変更のみ |
| 複数選択して移動/削除（カード優先） | 計画外・新規機能 | 移行元プラグインに存在しない、独自設計が必要 |
| GM以外のメニューから一部項目非表示 | 3-2 `hide-menu-*`（image/inventory/save） | 未移植 |
| 操作音のオンオフ | 3-4 `toggle-sound-effect` | 未移植 |
| 視点リセット→2D表示切り替え | 3-4 `reset-point-of-view` | 未移植 |
| カードをn枚引く | 3-1 `add-draw-n-cards` | 未移植 |
| カードタップ機能（右クリックメニューにも追加） | 実装済み `isTapCard` | 本体は対応不要、コンテキストメニュー項目の有無のみ要確認 |
| 足元の黄色い丸枠の表示/非表示（一括でよい） | 3-3 `hide-pedestal` と同一機能の可能性 | 要確認（「台座」と「丸枠」が指す対象が一致するか） |
| ポップアップ時のHP〜精神力を非表示 | 計画外・新規調査 | 移行元に該当プラグインなし、独自設計が必要 |
| 新規キャラクター作成時のステータス欄を空白に | 3-3 `empty-new-character` | 未移植（そのまま合致） |
| キーボードショートカット追加（コピー/ペースト/デリート） | 実装済み `isUseKeyboardShortcut`（基盤） | ショートカットの中身がコピペデリートを含むか要確認 |
| 右クリックメニューのアイコン化 | 3-2 `context-menu-add-icon` | 未移植 |
| "すべて正位置にする"をデフォルトに | 実装済み機能 `shuffle-normal` の初期値変更 | 区分A'（初期値・プリセット調整）で対応 |
| 「接続」内に退室ボタンを追加 | 3-2 `add-reload-button` | 未移植 |
| 地形作成時「回転オフ・点滅なし」をデフォルトに | 区分A'（依存関係・初期値調整） | 既存フラグの初期値/連動ルールで対応可能 |
| デフォルト地形データをURLのものと差し替え | 計画外（アセット差し替え作業） | コード実装ではなくアセット準備が先 |
| サンプル非表示（背景画像含む） | 3-3 `empty-default-objects` / `empty-default-table` | 未移植 |
| オリジナル初期画面に差し替え | 計画外（アセット未作成） | デザイン素材待ちでブロック中 |
| オフラインモード | 3-4 `offline-mode` | 未移植・Cloudflare構成での要否確認が必要（6章） |
