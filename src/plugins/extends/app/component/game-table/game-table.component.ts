...
import { GridLineRender } from 'src/app/component/game-table/grid-line-render';
import { TableMouseGesture } from 'src/app/component/game-table/table-mouse-gesture';
...
export class GameTableComponentExtendPlus implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngOnInit() {
    ...
    if(!pluginConfig.isEmptyDefaultTabletopObjects) this.tabletopActionService.makeDefaultTabletopObjects();
  }
  ...
  async onContextMenu(e: any) {
    ...
    const spreadSheetMenu = await onContextMenuDeckFromSpreadSheet(this, objectPosition);
    menuActions.push(...spreadSheetMenu);
    menuActions.push(ContextMenuSeparator);
    menuActions.push({
      name: 'テーブル設定', action: () => {
