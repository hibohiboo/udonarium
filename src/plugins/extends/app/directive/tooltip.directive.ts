...
import { OverviewPanelComponentExtendPlus } from '../component/overview-panel/overview-panel.component';
...
export class TooltipDirectiveExtendPlus implements OnInit, AfterViewInit, OnDestroy {
  ...
  private tooltipComponentRef: ComponentRef<OverviewPanelComponentExtendPlus>
  ...
  private close() {
    if (!this.tooltipComponentRef) return;
    let index = TooltipDirectiveExtendPlus.activeTooltips.indexOf(this.tooltipComponentRef);
    if (0 <= index) TooltipDirectiveExtendPlus.activeTooltips.splice(index, 1);
  ...
  private closeAll() {
    TooltipDirectiveExtendPlus.activeTooltips.forEach(componentRef => componentRef.destroy());
    TooltipDirectiveExtendPlus.activeTooltips = [];
    this.close();
  }
