...
@Injectable()
export class PanelServiceExtnedPlus {
  /* Todo */
  ...
  open<T>(childComponent: Type<T>, option?: PanelOption, parentViewContainerRef?: ViewContainerRef): T {
    if (!parentViewContainerRef) {
      parentViewContainerRef = PanelServiceExtnedPlus.defaultParentViewContainerRef;
    }
    ...
    panelComponentRef = parentViewContainerRef.createComponent(PanelServiceExtnedPlus.UIPanelComponentClass, {index: parentViewContainerRef.length, injector: parentViewContainerRef.injector});
    let bodyComponentRef: ComponentRef<any> = panelComponentRef.instance.content.createComponent(childComponent);

    const childPanelService: PanelServiceExtnedPlus = panelComponentRef.injector.get(PanelServiceExtnedPlus);
