...
export const extendsDiceSymbolComponent = (that: any) => {
  ...
  constructor.prototype.ngAfterViewInit = function() {
    ...
    if (pluginConfig.isOffObjectRotateIndividually) {
      ...
      const updateRotateOffClass = createClassUpdater('object-rotate-off', function() {
        return this.diceSymbol?.isRotateOffIndividually === true;
      });
  ...
  // ngOnDestroyは特に変更不要（元のコンポーネントで既にcleanupされている）
  ...
  constructor.prototype.makeContextMenu = function() {
    ...
    if (pluginConfig.isOffObjectRotateIndividually) {
      ...
      const isRotateOff = diceSymbol.isRotateOffIndividually;
      actions.push(ContextMenuSeparator);
      actions.push({
        name: isRotateOff ? '回転を有効にする' : '回転を無効にする',
        action: () => {
          diceSymbol.isRotateOffIndividually = !isRotateOff;
          // クラスを更新
