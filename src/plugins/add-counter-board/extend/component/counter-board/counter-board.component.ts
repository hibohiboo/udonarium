...
export class CounterBoardComponent implements OnInit, OnDestroy {
  ...
  get size() {
    return this.counterBoard.size;
  }
  get rightCorner() {
    return this.counterBoard.rightCorner;
  }
  get lowerRightCorner() {
    return this.counterBoard.lowerRightCorner;
  }
  get lowerLeftCorner() {
    return this.counterBoard.lowerLeftCorner;
  }
  get startPositionX() {
    return this.counterBoard.startPositionX;
  }
  get startPositionY() {
    return this.counterBoard.startPositionY;
  }
  get maxCount() {
    return this.counterBoard.maxCount;
  }
  get name() {
    return this.counterBoard.name;
  }
  get direction() {
    return this.counterBoard.direction;
  }
  get samePositionDisplay() {
    return this.counterBoard.samePositionDisplay;
  }
  get inRadius() {
    return this.counterBoard.inRadius;
  }
  get outRadius() {
    return this.counterBoard.outRadius;
  }
  get bufferDegree() {
    return this.counterBoard.bufferDegree;
  }
  ...
  get objects() {
    ...
    return this.tabletopService.terrains
      .filter((obj) => !!obj.detailDataElement?.getFirstElementByName(name))
      .map((obj) => {
        const countElement =
          obj.detailDataElement.getFirstElementByName(DETAIL_COUNT_NAME);
        const inOutElement =
          obj.detailDataElement.getFirstElementByName(IN_OUT_NAME);
        ...
        return {
          ...
          get currentValue() {
            return inOutElement?.currentValue;
          },
  ...
  trackByItem(
    ...
    return value ? value.obj.identifier : null;
  }
  delete() {
    if (
      !confirm(
        `カウンターボード ${this.counterBoard.name} を削除してもよいですか？`,
      )
    )
      return;
  ...
  addCube() {
    ...
    const cube = createDefaultCubeTerrain(
      { x: 0, y: 0, z: 0 },
      cubeProp.name,
      cubeProp.url,
    );
    const counterNameElement = DataElement.create(
      this.name,
      '',
      {},
      `${this.name}${cube.identifier}`,
    );
    cube.detailDataElement.appendChild(counterNameElement);
    counterNameElement.appendChild(
      DataElement.create(
        DETAIL_COUNT_NAME,
        '0',
        {},
        `${DETAIL_COUNT_NAME}${cube.identifier}`,
      ),
    );
    if (this.direction === 'diaclock')
      counterNameElement.appendChild(
        DataElement.create(
          IN_OUT_NAME,
          1,
          { type: 'numberResource', currentValue: '1' },
          `${DETAIL_COUNT_NAME}${cube.identifier}`,
        ),
      );
...
const updatePositionX = (obj, count, that, sign: 1 | -1) => {
  if (that.samePositionDisplay === 'stack')
    return updatePositionStack(obj, count, that, calcNextXDirection(sign));
...
const updatePositionDiaclock = (obj, count, that) => {
  ...
  const yBias = (-that.size * obj.depth) / 2; // 地形のY方向はheightではなくdepth
  const inOutElement = obj.detailDataElement.getFirstElementByName(IN_OUT_NAME);
  const radius =
    inOutElement.currentValue === '1' ? that.outRadius : that.inRadius;
  const calcNextXYDiaclock = calcNextXYDiaclockFactory(
    xBias,
    yBias,
    radius,
    Number(that.bufferDegree),
  );
...
const updatePositionStack = (obj, count, that, calc) => {
  ...
  // 上に積まれているオブジェクトを取得
  const currentPositionObjects = that.tabletopService.terrains.filter(
    (item) =>
      !!item.detailDataElement.getFirstElementByName(that.name) &&
...
const updatePositionXDirection = (obj, count, that, calc) => {
  const size = that.size;
  const sort =
    that.samePositionDisplay === 'bottom'
  ...
  obj.location.y =
    topObject == null
      ? that.startPositionY
      : calcNextDepth(topObject, obj, size, sign);
  obj.posZ = 0;
...
const updatePositionYDirection = (obj, count, that, calc) => {
  const size = that.size;
  const sort =
    that.samePositionDisplay === 'right'
  ...
  obj.location.x =
    topObject == null
      ? that.startPositionX
      : calcNextWidth(topObject, obj, size, sign);
  obj.posZ = 0;
...
const calcNextDepth = (topObject, self, size, sign: 1 | -1) => {
  const topValue =
    topObject.commonDataElement.getFirstElementByName('depth').value;
  if (sign > 0) {
...
const calcNextWidth = (topObject, self, size, sign: 1 | -1) => {
  const topValue =
    topObject.commonDataElement.getFirstElementByName('width').value;
  if (sign > 0) {
...
const calcNextHeight = (topObject, size) => {
  const topHeight =
    topObject.commonDataElement.getFirstElementByName('height').value;
...
const calcNextXY = (count, that) => {
  ...
  const remainder = count % (that.maxCount + 1);
  const nextYCount = calcNextY(
    remainder,
    rightCorner,
    lowerRightCorner,
    lowerLeftCorner,
  );
  const nextXcount = calcNextX(
    remainder,
    rightCorner,
    lowerRightCorner,
    lowerLeftCorner,
...
const calcNextX = (count, rightCorner, lowerRightCorner, lowerLeftCorner) => {
  ...
  if (count >= lowerRightCorner)
    return rightCorner - (count - lowerRightCorner);
  return rightCorner;
};
const calcNextY = (count, rightCorner, lowerRightCorner, lowerLeftCorner) => {
  if (count <= rightCorner) return 0;
  if (count >= lowerLeftCorner)
    return lowerRightCorner - rightCorner - (count - lowerLeftCorner);
  if (count > lowerRightCorner) return lowerRightCorner - rightCorner;
  return count - rightCorner;
};

const calcNextXYDiaclockFactory =
  (xBias: number, yBias: number, radius: number, bufferDegree: number) =>
  (count, that) => {
