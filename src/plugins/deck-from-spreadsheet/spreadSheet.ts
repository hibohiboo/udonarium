import { getSpreadSheetAPIKey } from './extend/app.component'
...
export const getBookData = async (
  ...
  const url = `${fetchUrl}/${spreadId}?key=${getSpreadSheetAPIKey()}`
...
export const getSheetData = async (
  ...
  const res = await fetch(
    `${fetchUrl}/${spreadId}/values/${sheet}!${range}?key=${getSpreadSheetAPIKey()}`,
  )
  if (res.status >= 400) {
