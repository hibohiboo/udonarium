...
export class BlankCardComponent implements OnDestroy, OnChanges, AfterViewInit {
  @Input() card: BlankCard = null;
  ...
  onCardDrop(e) {
    if (this.card === e.detail || (e.detail instanceof BlankCard === false && e.detail instanceof BlankCardStack === false)) {
      return;
    }
    e.stopPropagation();
    e.preventDefault();

    if (e.detail instanceof BlankCardStack) {
  ...
  onContextMenu(e: Event) {
    ...
    this.contextMenuService.open(position, menuActions, this.isVisible ? this.name : 'ブランクカード');
  }
  ...
  private createStack() {
    let cardStack = BlankCardStack.create('山札');
    cardStack.location.x = this.card.location.x;
    cardStack.location.y = this.card.location.y;
    cardStack.posZ = this.card.posZ;
    ...
    let cards: BlankCard[] = (this.tabletopService as any).blankCards.filter((card: BlankCard) => {
  ...
  private makeSelectionContextMenu(): ContextMenuAction[] {
    ...
    if (this.isSelected) {
      let selectedCards = () => this.selectionService.objects.filter(object => object.aliasName === this.card.aliasName) as BlankCard[];
      actions.push(
        {
  ...
  private showDetail(gameObject: BlankCard) {
