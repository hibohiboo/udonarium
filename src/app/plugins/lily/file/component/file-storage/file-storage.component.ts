...
import { ImageTag } from '../../class/image-tag';//本家PR #92より
...
export class FileStorageComponentLily implements OnInit, OnDestroy, AfterViewInit {
...
//本家PR #92より
  ...
  get images(): ImageFile[] {
      ...
      for (let imageFile of this.fileStorageService.images){
        let identifier = imageFile.identifier;
  ...
  changeTag(){
   ...
   for (let identifier of this.identifierList) {
     ...
     for( let changeableImage of changeableImages ){
       if( changeableImage.identifier == identifier ){
