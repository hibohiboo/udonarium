  ...
  cardStackComponentOnContextMenuHook: (that, position: PointerData) => {
    ...
    if (that.cards.length > 0) {
      ...
      actions.push({
        name: 'カード一覧を見る',
        action: () => {
          that.showStackList(that.cardStack)
        },
      })
      actions.push(ContextMenuSeparator)
      if (that.isShowTotal) {
        actions.push({
          name: '☑ 枚数を表示',
          action: () => {
            that.cardStack.isShowTotal = false
          },
        })
      } else {
        actions.push({
          name: '☐ 枚数を表示',
          action: () => {
            that.cardStack.isShowTotal = true
          },
        })
      }
      actions.push({
        name: 'カードサイズを揃える',
        action: () => {
          if (that.cardStack.topCard)
            that.cardStack.unifyCardsSize(that.cardStack.topCard.size)
        },
      })
      actions.push(ContextMenuSeparator)
      actions.push({
    ...
    actions.push({
      name: 'コピーを作る',
      action: () => {
        const cloneObject = that.cardStack.clone()
