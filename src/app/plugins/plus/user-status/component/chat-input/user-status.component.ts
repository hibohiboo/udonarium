...
import { chatInputAllowsChatHook } from 'src/app/plugins'
...
export class UserStatusComponent implements OnInit, OnDestroy {
  ...
  private _speechThreshold = 50
  get speechThreshold() {
    return this._speechThreshold
  }
  set speechThreshold(val) {
    this._speechThreshold = val
  }
  private _volume = 0
  get volume() {
    return this._volume
  ...
  get otherPeers(): PeerCursor[] {
    return ObjectStore.instance
      ...
      .filter((peer) => peer.peerId !== this.myPeer.peerId)
  }
  ...
  ngOnInit(): void {
    ...
    Promise.resolve().then(
      () => (this.modalService.title = this.panelService.title = '発言状態'),
    )
    ...
    EventSystem.register(this)
      .on('UPDATE_GAME_OBJECT', -1000, (event) => {
        ...
        const gameCharacter = ObjectStore.instance.get<GameCharacter>(
  ...
  private updatespeechingPeerNames() {
    this.speechingPeerNames = Array.from(this.speechingPeers.keys()).map(
      (peerId) => {
  ...
  get isSpeechConnect() {
    return this._isSpeechConnect
  }
  set isSpeechConnect(b) {
    this._isSpeechConnect = b
  }
    ...
    const initAudio = async (callback) => {
      ...
      try {
        ...
        const getByteFrequencyDataAverage = () => {
          ...
          return (
            frequencies.reduce((previous, current) => previous + current, 0) /
  ...
  private allowsChat(gameCharacter: GameCharacter): boolean {
    const hookResult = chatInputAllowsChatHook(
      gameCharacter,
      this.myPeer.peerId,
    )
  ...
  findSpeechName(peer: PeerCursor) {
    const character = this.gameCharacters.find(
      (g) => g.identifier === peer.speechIdentifier,
    )
    if (!character) return ''
  ...
  private getCharacterSpeechImg(peer: PeerCursor): ImageFile[] {
    const object = ObjectStore.instance.get(peer.speechIdentifier)
    ...
    const [speeching1, speeching2] = object.imageDataElement.getElementsByName(
      '発言中',
    )
    if (!speeching1) {
      return [object.imageFile]
    }
    const image1: ImageFile = ImageStorage.instance.get(
      <string>speeching1.value,
    )

    if (!speeching2) {
      return [image1]
    }
    const image2: ImageFile = ImageStorage.instance.get(
      <string>speeching2.value,
    )
